...
public final class PoolAdditionMerger {
    ...
    public static void mergeAdditionPools(final ServerGoingToStartEvent event) {
        ResourceManager resourceManager = ((StructureTemplateManagerAccessor) event.getServer().getStructureManager()).mes_getResourceManager();
    ...
    private static void mergeIntoExistingPool(AdditionalStructureTemplatePool feedingPool, StructureTemplatePool gluttonyPool, StructureTemplateManager structureTemplateManager) {
        ...
        ObjectArrayList<StructurePoolElement> elements = new ObjectArrayList<>(((StructurePoolAccessor) gluttonyPool).mes_getTemplates());
        List<Pair<StructurePoolElement, Integer>> elementCounts = new ArrayList<>(((StructurePoolAccessor) gluttonyPool).mes_getRawTemplates());

        elements.addAll(((StructurePoolAccessor) feedingPool).mes_getTemplates());
        elementCounts.addAll(((StructurePoolAccessor) feedingPool).mes_getRawTemplates());
        ...
        for(StructurePoolElement element : elements) {
            if(element instanceof SinglePoolElement singlePoolElement) {
                Optional<ResourceLocation> pieceRL = ((SinglePoolElementAccessor)singlePoolElement).mes_getTemplate().left();
        ...
        ((StructurePoolAccessor) gluttonyPool).mes_setTemplates(elements);
